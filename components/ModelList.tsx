...
export function ModelList({ dataModels, favoritos = [], toggleFavorito, isLoggedIn = false }: ModelListProps) {
    ...
    return (
        <section className="grid grid-cols-1 lg:grid-cols-2 justify-center items-center gap-10 py-10 px-5 bg-white">
            {dataModels.map((data, index) => (
                <div key={index} className="relative border-2 border-primary rounded-lg p-4 gap-4 lg:w-[550px] w-full lg:h-80">
                    <Link
                        href={isLoggedIn ? `/cliente/${data.nombreUsuario}` : `/${data.nombreUsuario}`}
                    ...
                    >
                        ...
                        <div className="flex flex-col w-full">
                            <h3 className="text-xl font-bold tracking-tight text-gray-900">{data.nombreCompleto}</h3>
                            ...
                            <span className="text-gray-500 font-semibold py-10">{data.zona}</span>
                        </div>
                    </Link>
                    ...
                    {isLoggedIn && toggleFavorito && (
                        ...
                        >
                            <Image
                                src={favoritos.some((fav) => fav.nombreUsuario === data.nombreUsuario) ? "/star-solid.svg" : "/star.svg"}
                                width={30}
                                height={30}
                                alt="Icon Star"
                            />
