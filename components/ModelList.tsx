...
export function ModelList({ dataModels }: ModelListProps) {
    ...
    return (
        <section className="grid grid-cols-1 lg:grid-cols-2 justify-center items-center gap-10 py-10 px-5 bg-white">
            {dataModels.map((data, index) => (
                <Link key={index} href={`/${data.nombreUsuario}`} className="grid grid-cols-1 lg:grid-cols-2 border-2 border-primary rounded-lg p-4 gap-4 lg:w-[550px] w-full lg:h-80">
                    {/* Columna imagen */}
